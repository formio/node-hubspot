...
hubspotAPI.prototype.execute = function (verb, path, availableParams, givenParams, callback) {
  ...
  if (verb === "POST") {
    var properties = [];
    for (var key in finalParams.properties) {
      properties.push({
        property: key,
        value: finalParams.properties[key]
      });
    }
...
hubspotAPI.prototype.contacts_contact_by_id = function(params, callback) {
  var version = params.version ? params.version : "v1";
  var path = "contacts/" + version + "/contact/vid/" + params.vid + "/profile/",
    availableParams = [];

  this.get(path, availableParams, params, callback);
};

hubspotAPI.prototype.contacts_contact_by_email = function(params, callback) {
  var version = params.version ? params.version : "v1";
  var path = "contacts/" + version + "/contact/email/" + params.email + "/profile/",
    availableParams = [];
